      ...
      <div id="wrapper">
        ...
        <div id="Schedule" class="tabcontent">
          ...
          <div style="display:inline-block">
            ...
            <button class="greenBtn" onclick="createNewScheduleWindow(-1,null);">ADD NEW</button>
          </div>
        </div>
...
DynamicJsonDocument SchedulesJson(6024);

JsonObject schedules = SchedulesJson.createNestedObject("Schedules");
JsonArray timeSchedule = schedules.createNestedArray("timeSchedule");
...
DynamicJsonDocument colorListJson(2048);
JsonArray colorList = colorListJson.createNestedArray("colorListBreathing");
...
void setAnimation(String animation){
    if(playingAnimation != animation){
      if(animation == "Power Off"){
        PowerState = false;
        turnOffAnimations();
        webSocket.broadcastTXT("{\"Animations\":{\"playingAnimation\":\"Power Off\",\"PowerState\":"+String(PowerState ? "true":"false")+"}}");
      ...
      webSocket.broadcastTXT("{\"Animations\":{\"playingAnimation\":\""+playingAnimation+"\",\"PowerState\":"+String(PowerState ? "true":"false")+"}}");
...
void setAnimationParameters(JsonObject &parameters){
    ...
    if (parameters.containsKey("Breathing"))
    {
      ...
      if(Breathing.containsKey("colorListBreathing")){
          colorListJson.clear();
          colorList = colorListJson.createNestedArray("colorListBreathing");
      ...
      if(Breathing.containsKey("addColortoList")) {
        if(colorList.size()!=25){
          ...
          webSocket.broadcastTXT("{\"Animations\":{\"parameters\":{\"Breathing\":{\"colorListBreathing\":"+colorListJson["colorListBreathing"].as<String>()+"}}}}");
        }
      }
      ...
      {
        JsonObject Disco = parameters["Disco"];
      ...
      {
        JsonObject SolidDisco = parameters["SolidDisco"];
      ...
      {
        JsonObject MorseCode = parameters["MorseCode"];  
        ...
        if(MorseCode.containsKey("encodedMorseCode")) {    
          if (strlen(encodedMessage) > 0) free(encodedMessage);
      ...
      {
        JsonObject Ambilight = parameters["Ambilight"];
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length)
{
  if(type == WStype_CONNECTED){
      ...
      if(powerConected) setAnimation("Power On");
  ...
  if (type == WStype_TEXT)
  {
        ...
        if (Animations.containsKey("parameters")){
          JsonObject parameters = Animations["parameters"];
          setAnimationParameters(parameters);
        }      
        if (Animations.containsKey("powerConected")) powerConected = Animations["powerConected"];
        if (Animations.containsKey("Schedules")){
          JsonObject _schedules = Animations["Schedules"];   
          if (_schedules.containsKey("timeSchedule")){    
              schedules["timeSchedule"].clear();
              scheduler.schedules.clear();
              timeSchedule = schedules.createNestedArray("timeSchedule");
              
              for (size_t j = 0; j < _schedules["timeSchedule"].size(); j++)
              {
                timeSchedule.add(_schedules["timeSchedule"][j]);  
                int x = timeSchedule.size()-1;
                String label = timeSchedule[x]["Label"];       
                String anim = timeSchedule[x]["playingAnimation"];
                unsigned long timestamp = timeSchedule[x]["Timestamp"];
                bool days[7];
                for (size_t i = 0; i < 7; i++) days[i] = timeSchedule[x]["Days"][i];      
                ScheduleTask task = ScheduleTask(label, days, timestamp,  
                  [=](){    
                    for (JsonArray::iterator it=timeSchedule.begin(); it!=timeSchedule.end(); ++it) {
                      if ((*it)["Timestamp"] == timestamp) {
                        ...
                        setAnimation(anim); 
                        String out = "{\"Animations\":{\"parameters\":" + (*it)["parameters"].as<String>()+"}}";
                        webSocket.sendTXT(num,out);
                        break;                     
                      }
                    }
                  });
                task.enable = timeSchedule[x]["enable"];
                scheduler.add(task);
              }  
              webSocket.broadcastTXT("{\"Animations\":{\"Schedules\":{\"timeSchedule\":"+SchedulesJson["Schedules"]["timeSchedule"].as<String>()+"}}}");
          }
          if (_schedules.containsKey("newTimeSchedule")){
            if (timeSchedule.size()<=10){    
              timeSchedule.add(_schedules["newTimeSchedule"]);  
              int x = timeSchedule.size()-1;
              String label = timeSchedule[x]["Label"];       
              String anim = timeSchedule[x]["playingAnimation"];
              unsigned long timestamp = timeSchedule[x]["Timestamp"];
              ...
              Serial.println("Can't add more than 15 time schedules!");
            }  
          }
          if (_schedules.containsKey("removeTimeSchedule")){
            ...
            {
              if (timeSchedule[i]["Timestamp"] == removeSchedule["Timestamp"]){
                timeSchedule.remove(i);
                scheduler.Remove(i);
                webSocket.broadcastTXT("{\"Animations\":{\"Schedules\":{\"timeSchedule\":"+SchedulesJson["Schedules"]["timeSchedule"].as<String>()+"}}}");
                break;
              }
              ...
              for (size_t i = 0; i < timeSchedule.size(); i++)
              {
                if (timeSchedule[i]["Timestamp"] == _schedules["editTimeSchedule"]["oldTimestamp"]){
                  if (newdata.containsKey("Label")) {
                    timeSchedule[i]["Label"] = newdata["Label"];
                    scheduler.schedules[i].label = newdata["Label"].as<String>();
                  }
                  if (newdata.containsKey("Days")) {                  
                    timeSchedule[i]["Days"] = newdata["Days"];           
                    for (size_t x = 0; x < 7; x++) scheduler.schedules[i].days[x] = newdata["Days"][x];
                  }
                  if (newdata.containsKey("Timestamp")) {
                    timeSchedule[i]["Timestamp"] = newdata["Timestamp"];
                    scheduler.schedules[i].timestamp = newdata["Timestamp"];
                    scheduler.schedules[i].taskCompleted = false;
                  }  
                  if (newdata.containsKey("playingAnimation") && newdata.containsKey("parameters")) {             
                    String anim = newdata["playingAnimation"]; 
                    timeSchedule[i]["playingAnimation"] = anim;
                    timeSchedule[i]["parameters"] = newdata["parameters"];
                  ...
                  if (newdata.containsKey("enable")) {
                    timeSchedule[i]["enable"] = newdata["enable"];
                    scheduler.schedules[i].enable = newdata["enable"];
                  }
                  webSocket.broadcastTXT("{\"Animations\":{\"Schedules\":{\"timeSchedule\":"+SchedulesJson["Schedules"]["timeSchedule"].as<String>()+"}}}");
                  break;
                }
          ...
          if (_schedules.containsKey("enableScheduler")) enableScheduler = _schedules["enableScheduler"];
        }
        if (Animations.containsKey("playingAnimation")) setAnimation(Animations["playingAnimation"]);                        
  ...
  if(type == WStype_DISCONNECTED){
    ...
    if(webSocket.connectedClients() == 0) {
      if(powerConected) setAnimation("Power Off");
      startSaving = false;
    }
    saveUserData();
  }
...
DynamicJsonDocument AnimationsJson(){
  DynamicJsonDocument doc(14240);
    JsonObject Animations = doc.createNestedObject("Animations");
      JsonObject parameters = Animations.createNestedObject("parameters");
          ...
          JsonObject Breathing = parameters.createNestedObject("Breathing");
            ...
            JsonArray _colorListBreathing = Breathing.createNestedArray("colorListBreathing");
              _colorListBreathing.set(colorListJson["colorListBreathing"].as<JsonArray>());
          JsonObject Disco = parameters.createNestedObject("Disco");
      ...
      Animations["Schedules"] = schedules;
      Animations["Schedules"]["enableScheduler"] = enableScheduler;
...
void saveConfig(){  
  DynamicJsonDocument json(17240);
...
bool loadConfig() {
  ...
  DynamicJsonDocument json(19240);
  ...
  if(json.containsKey("Animations")) {
    ...
    if (_Animations.containsKey("parameters")){
      ...
      if(parameters.containsKey("Breathing")){
        ...
        if(_Breathing.containsKey("colorListBreathing")) colorList.set(_Breathing["colorListBreathing"].as<JsonArray>());       
    ...
    if(_Animations.containsKey("Schedules")) {
        ...
        enableScheduler = _Schedules["enableScheduler"];

        if(_Schedules.containsKey("timeSchedule")){
        JsonArray _timeSchedule = _Schedules["timeSchedule"];     
        for (size_t i = 0; i < _timeSchedule.size(); i++)
        {
          JsonObject newSchedule = _timeSchedule[i];
          String label = newSchedule["Label"];
          String anim = newSchedule["animationToPlay"];
          unsigned long timestamp = newSchedule["Timestamp"];
          JsonObject param = newSchedule["parameters"];        
          bool days[7];
          for (size_t i = 0; i < 7; i++) days[i] = newSchedule["Days"][i];      
          ScheduleTask task = ScheduleTask(label, days, timestamp,  
            [=](){ 
              JsonObject paramseters = param;
              if(paramseters.size()>0) setAnimationParameters(paramseters); 
              setAnimation(anim); 
              });
...
void loop() { 
  ...
  if(initialized) {
    ArduinoOTA.handle();
    webSocket.loop();
    if(enableScheduler) scheduler.update();       
...
/*
   if (currentMillis - flashmillis >= 15000) {
